...
class ProxyRequestsAsync:
    ...
    async def __acquire_sockets(self):
        r = await requests_async.get("https://www.sslproxies.org/")
    ...
    async def get(self):
        if len(self.sockets) > 0 and self.try_count > 0:
            ...
            try:
                request = await requests_async.get(
                    self.url,
                    timeout=3.0,
                    proxies=proxies
                )
    ...
    async def get_with_headers(self):
        if len(self.sockets) > 0 and self.try_count > 0:
            current_socket = self.sockets.pop(0)
            proxies = {
                "http": "http://" + current_socket,
    ...
    async def post(self, data):
        if len(self.sockets) > 0 and self.try_count > 0:
            ...
            try:
                request = await requests_async.post(
                    self.url,
                    json=data,
                    timeout=3.0,
                    proxies=proxies
    ...
    async def post_with_headers(self, data):
        if len(self.sockets) > 0 and self.try_count > 0:
            current_socket = self.sockets.pop(0)
            proxies = {
                "http": "http://" + current_socket,
    ...
    async def post_file_with_headers(self):
        if len(self.sockets) > 0 and self.try_count > 0:
            current_socket = self.sockets.pop(0)
            proxies = {
                "http": "http://" + current_socket,
...
class ProxyRequestsAsyncBasicAuth(ProxyRequestsAsync):
    ...
    @classmethod
    async def create(cls, url, username, password):
        self = ProxyRequestsAsyncBasicAuth(url, username, password)
        await self.__acquire_sockets()
        return self
